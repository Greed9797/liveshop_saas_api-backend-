...
export async function financeiroRoutes(app) {
  ...
  app.get('/v1/financeiro/resumo', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request) => {
  ...
  app.get('/v1/financeiro/faturamento', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request) => {
    const { tenant_id } = request.user
  ...
  app.get('/v1/financeiro/fluxo-caixa', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request) => {
    const { tenant_id } = request.user
    const { mes, ano } = request.query
    const periodo = mes && ano
      ? `${ano}-${String(mes).padStart(2, '0')}-01`
  ...
  app.post('/v1/financeiro/custos', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request, reply) => {
  ...
  app.get('/v1/financeiro/custos', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request) => {
    const { tenant_id } = request.user
  ...
  app.delete('/v1/financeiro/custos/:id', { preHandler: app.requirePapel(['franqueado', 'franqueador_master', 'gerente']) }, async (request, reply) => {
    const { tenant_id } = request.user
    const db = await app.dbTenant(tenant_id)
    try {
      const result = await db.query(
